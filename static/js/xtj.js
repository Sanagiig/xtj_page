...
(function () {
    ...
    //维持goEasy 心跳
    function heartbeat() {
        $.get(host + "/data/heartbeat", {}, function (data) {
            console.log("Heart beat response")
            console.log(data)
        });
    }
    ...
    function createGoEasy() {
        var goEasy = new GoEasy({appkey: 'BC-33d16fcff16b4ca0937edea9c126900f'}),
            pCount = 0,     //当前热地图发送的次数
            pLimit = 13, //热地图 数据的数量
            hotTempPoints = [];

        goHeartBeatInterv = setInterval(heartbeat, 10000);
        goEasy.subscribe({
